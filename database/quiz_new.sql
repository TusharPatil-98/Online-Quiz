...
CREATE TABLE IF NOT EXISTS `mst_question` (
  ...
  `true_ans` varchar(75) DEFAULT NULL,
  PRIMARY KEY (`que_id`)
...
(1, 'tushar', 'anything', 'tushar', 'belgaum', 'belgaum', 8795462130, 'tushar@yahoo.com'),
(2, 'namskar', 'namskar', 'namskar', 'belgaum','belgaum', 5468792130, 'namskar@yahoo.com');
*/
-- --------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `mst_useranswer` (
  ...
  `true_ans` varchar(50) DEFAULT NULL,
  `your_ans` varchar(50) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE PROCEDURE `deleteQuestion`(IN `id` INT)
 ...
 DELETE FROM mst_question WHERE que_id = id;

CREATE TRIGGER `mst_backup`
 BEFORE DELETE ON `mst_question`
 FOR EACH ROW
 INSERT INTO backup values(OLD.que_id,OLD.test_id,OLD.que_desc,OLD.ans1,OLD.ans2,OLD.ans3,OLD.ans4,OLD.true_ans);
