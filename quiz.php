...
{	
		if($submit=='Next Question' && isset($ans))
		{
				...
				//print_r($row);
				//echo $ans;
				mysql_query("insert into mst_useranswer(sess_id, test_id, que_des, ans1,ans2,ans3,ans4,true_ans,your_ans) values ('".session_id()."', $tid,'".$row[2]."','".$row[3]."','".$row[4]."','".$row[5]."', '".$row[6]."','".$row[7]."','$row[$ans]')") or die(mysql_error());
				if($row[$ans]==$row[7])
				{
...
if($_SESSION[qn]<mysql_num_rows($rs)-1)
echo "<tr><td><input class='btn btn-primary' type=submit name=submit value='Next Question'></form>";
else
echo "<tr><td><input class='btn btn-success' type=submit name=submit value='Get Result'></form>";
echo "</table></table>";
?>
</body>
</html>